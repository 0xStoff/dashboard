...
function ManageWallets() {
  ...
  const [editingWallet, setEditingWallet] = useState<{ id: string; tag: string; wallet: string; chain: string; show_chip: boolean } | null>(null);
  ...
  const handleToggleShowChip = async (id: string, currentValue: boolean) => {
    try {
      await axios.put(`${process.env.REACT_APP_API_BASE_URL}/wallets/${id}`, { show_chip: !currentValue });
